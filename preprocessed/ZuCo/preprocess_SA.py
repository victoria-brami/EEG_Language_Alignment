...
TASKS = ["task1-SR", "task3-TSR"]
patients = ['ZAB', 'ZDM', 'ZDN', 'ZJM', 'ZJN', 'ZJS', 'ZKH', 'ZKW', 'ZMG']

for task_name in TASKS:
    for patient in patients:
        file_name = f"{task_name}/Matlab_files/results{patient}_{task_name.split("-")[-1]}.mat"
        ...
        df.to_csv(f'{patient}_sentence.csv')


        sentiment = pd.read_csv('label.csv')

        read = pd.read_csv(f'{patient}_sentence.csv')
        ...
        data.to_csv(f'{patient}_sentence.csv', index = False)
        ...
        for i in range(len(data)):
            ...
            df.to_csv(f'{patient}{count}_word.csv', index=False)
            count+=1
        ...
        for i in sorted(os.listdir()):
            if '_word.csv' in i:
                ...
                df.to_csv(f'{patient}_word.csv', index = False)
        ...
        new_df.to_csv(f'{patient}_word.csv')

        df = pd.read_csv(f'{patient}_word.csv')
                
        data = df.loc[df['2'] !=0]

        data.to_csv(f'{patient}_word.csv', index = False)


        # --- concat word

        new_df = pd.DataFrame()
        for i in sorted(os.listdir(f"{patient}")):
            if '.csv' in i:
                read = pd.read_csv(f'{patient}/{i}')
                text = []
                df = pd.DataFrame()
